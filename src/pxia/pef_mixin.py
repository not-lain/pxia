...
class PeftAdapterMixin:
        ...
        if "device" not in adapter_kwargs:
            ...
            if hasattr(self, "device"):
                if not hasattr(self, "hf_device_map"):
                    ...
                    list(self.hf_device_map.values())[0]
            # Custom models
            else:
    ...
    def merge_and_unload(
        self, adapter_name: Optional[str] = None, progressbar: bool = False
    ) -> "torch.nn.Module":
        """
        Merge the adapter weights into the base model and remove the adapter. This is useful to save memory when you don't need
        the adapter anymore. The adapter weights will be merged into the base model's weights.

        Args:
            adapter_name (`str`, *optional*):
                The name of the adapter to merge. If None, the active adapter will be merged.
            progressbar (`bool`, *optional*, defaults to `False`):
                Whether to show a progressbar indicating the unload and merge process.

        Returns:
            `torch.nn.Module`: The base model with merged adapter weights.
        """
        check_peft_version(min_version=MIN_PEFT_VERSION)
        ...
        from peft.utils import merge_adapter_weights

        if adapter_name is None:
            adapter_name = self.active_adapters()[0]

        return merge_adapter_weights(self, adapter_name, progressbar=progressbar)
    ...
    def _dispatch_accelerate_model(
        ...
        )  # Get from model class if available
